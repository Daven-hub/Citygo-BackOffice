            ...
            <div ref={popoverRef} className='flex relative items-center font-bold text-[.75rem] gap-2'>
                  <div onClick={() => setOpen(!open)} className='cursor-pointer flex items-center gap-1.5'>
                        ...
                        <div className='flex flex-col leading-[1.3]'>
                              <span className={`${isAdmin?(isLight?'text-white/85':'text-black/75'):'text-black/75'} truncate max-w-[110px] text-[.92rem] capitalize font-semibold`}>{userConnected?.username ?userConnected?.username: userConnected?.prenom+' '+userConnected?.nom}</span>
                              <span className={`${isAdmin?(isLight?'text-white/75':'text-gray-500'):'text-gray-800/70'} capitalize font-light text-[.7rem]`}>{userConnected?.role}</span>
                        </div>
                        <ChevronDown />
                  </div>
                  {/* Popover */}
                  {open && (
                        <div className="absolute top-full right-0 overflow-hidden mt-2 w-[200px] bg-white border border-gray-200 rounded-lg shadow-lg z-20">
                              ...
                              <ul className="py-1 text-[.8rem] font-medium text-gray-500">
                                    {isAdmin && <li>
