...
export default function LanguageModal({open, setOpen, dispatch, selected, setSelected }) {
    ...
    const onSubmit = async (datas) => {
        ...
        try {
            if(!selected){
                await dispatch(createLanguage(newData)).unwrap();
                toast({
                    title: "Nouvel Enregistrement",
                    description: "La langue avec pour code "+newData.code+" a été créé avec success!",
                });
            }else{
                const newsData={id:selected?.id,datas:newData}
                await dispatch(updateLanguage(newsData)).unwrap();
                toast({
                    title: "Modification",
                    description: "La langue avec pour code "+newData.code+" a été modifié avec success!",
                });
            }
            setOpen(false)
            reset({})
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogContent>
                <DialogTitle className="text-[1.3rem] font-semibold">Nouvelle Langue</DialogTitle>
                ...
                <form id='typeVehicule' onSubmit={handleSubmit(onSubmit)} className="mt-2">
                    <div className="grid text-sm grid-cols-2 gap-3">
                        ...
                        <div className="col-span-2 space-y-0">
                            <label className="block text-sm font-medium text-gray-700">Nom Native</label>
                            <Input
                                type="text"
                                className="mt-1 block w-full rounded-md border p-2"
                                {...register("nativeName")}
