...
import { createLanguage, updateLanguage } from "@/store/slices/catalogue/language.slice"
import { createCurrencie, updateCurrencie } from "@/store/slices/catalogue/currencie.slice"

export default function CurrenciesModal({open, setOpen, dispatch, selected, setSelected }) {
    ...
    const onSubmit = async (datas) => {
        ...
        try {
            if(!selected){
                await dispatch(createCurrencie(newData)).unwrap();
                toast({
                    title: "Nouvel Enregistrement",
                    description: "La Monnaie avec pour code "+newData.code+" a été créé avec success!",
                });
            }else{
                const newsData={id:selected?.id,datas:newData}
                await dispatch(updateCurrencie(newsData)).unwrap();
                toast({
                    title: "Modification",
                    description: "La Monnaie avec pour code "+newData.code+" a été modifié avec success!",
                });
            }
            setOpen(false)
            reset({})
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogContent>
                <DialogTitle className="text-[1.3rem] font-semibold">Nouvelle Monnaie</DialogTitle>
