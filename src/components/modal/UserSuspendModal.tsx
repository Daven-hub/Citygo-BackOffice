...
import { getUserById, suspendUserById, UserType } from "@/store/slices/user.slice";
...
interface UserSuspendModalProps {
  ...
  // onSuspend?: (userId: string, data: SuspendFormValues) => void;
}
      ...
      <DialogContent className="sm:max-w-md bg-card border-border">
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-foreground">Suspendre l'utilisateur</DialogTitle>
        ...
        <div className="flex items-center gap-3 p-3 rounded-lg bg-warning/10 border border-warning/20">
          ...
          <p className="text-xs text-warning">
            Cette action empêchera l'utilisateur d'accéder à la plateforme pendant la durée spécifiée.
          </p>
        </div>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="space-y-0.5">
                  <FormLabel className="text-foreground">Raison de la suspension</FormLabel>
                  <FormControl>
                    <Textarea
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="space-y-0.5">
                  <FormLabel className="text-sm text-foreground">Durée (en jours)</FormLabel>
                  <FormControl>
                    <Input
                      type="number"
                      min={1}
