...
function DetailUserAdmin() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        await Promise.all([dispatch(getUserById(parseInt(userId))).unwrap()])
  ...
  return (
    <div className='flex flex-col gap-6'>
      ...
      {/* HEADER */}
      <div className='flex px-5 py-4 rounded-[6px] bg-white justify-between items-center border'>
        <h1 className='flex items-center text-gray-800 text-lg font-medium gap-2'>
          <UserIcon size={20} className='text-primary' /> Détail du compte
        </h1>
        <Breadcrumb />
      </div>

      {/* USER CARD */}
      <div className='w-full relative flex flex-col rounded-[6px] border bg-white overflow-hidden'>
        <div className='h-36 bg-gradient-to-r from-primary to-blue-500 relative'>
          <div className='absolute p-1.5 border bg-white shadow-lg rounded-full w-[110px] top-20 left-10 h-[110px]'>
        ...
        <div className='flex flex-col py-10 px-6 pl-36'>
          <h2 className='text-2xl font-bold text-gray-800'>{detail?.fullname}</h2>
          <p className='text-gray-500 text-sm'>{detail?.email}</p>
          <p className='text-gray-500 text-sm'>{detail?.phone}</p>
        </div>
      </div>

      {/* MAIN TABS */}
      <div className='w-full bg-white border rounded-xl p-6 shadow-lg'>
